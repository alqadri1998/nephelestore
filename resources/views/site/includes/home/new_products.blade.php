        ...
        @foreach ($categories as $category)
            @isset($category->products)
                @if (count($category->products) > 0)
                    <h2 class="section-title ls-n-15 text-center pb-2 m-b-4">{{ $category->name }}</h2>
                ...
                {{-- @else --}}
                    ...
                    {{-- <div class="col-md-3">
                        <div class="product-default inner-quickview inner-icon">
                            <figure>
                                <a href="{{ route('product.show', $new->slug) }}">
                                    <img src="{{ asset('assets/site/images/logo.jpeg') }}" width="273" height="273"
                                        alt="{{ $new->name }}" />
                                </a>
                                <div class="label-group">
                                ...
                                <div class="btn-icon-group">
                                    <button
                                        class="btn-icon btn-add-cart product-type-simple {{ $new->variants->count() > 0 ? '' : 'simple' }}"
                                        data-toggle="modal" data-target="#addCartModal" data-product-id="{{ $new['id'] }}"
