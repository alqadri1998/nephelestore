<form method="post" action="{{isset($item) ? route('admin.users.update',$item) : route('admin.users.store') }}" enctype="multipart/form-data">
    {{ csrf_field() }}
    @if(isset($item))
        @method('PATCH')
    @endif
    <div class="card-body">
        ...
        <div class="form-group half-width">
            ...
            <input type="email" dir="ltr" class="form-control" id="inputAddress" name="email" value="{{ old('email') ?? (isset($item) ? $item->email : '') }}" >
        </div>

        <div class="form-group half-width">
            <label for="inputAddress">{{ __('admin.admins.Mobile')}}</label>
            <input type="text" dir="ltr" class="form-control" id="inputAddress" name="mobile" value="{{ old('mobile') ?? (isset($item) ? $item->mobile : '') }}" required>
        </div>
        <div class="form-group half-width">
            <label for="inputAddress">{{ __('admin.admins.Password')}}</label>
            <input type="password" dir="ltr" class="form-control" id="inputAddress" name="password" {{ !isset($item) ? 'required' : '' }}>
        </div>
        <div class="row">
            <div class="col-md-6">
