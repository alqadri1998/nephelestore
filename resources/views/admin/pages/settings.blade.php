			...
			<form method="post" action="{{ route('admin.changeSetting') }}" enctype="multipart/form-data">
			    ...
			    <div class="card-body">
			    	<div class="row">
			    		...
			    		<div class="col-md-6">
					    	<div class="form-group">
					            ...
					            <input type="text" class="form-control" id="site_name_en" name="site_name_en" value="{{ old('site_name_en') ?? (isset($settings['site_name_en']) ? $settings['site_name_en'] : '') }}">
					        </div>
			    		</div>
			    		{{-- Email --}}
			    		<div class="col-md-6">
			    			<div class="form-group">
					            ...
					            <input type="text" class="form-control" id="email" name="email" value="{{ old('email') ?? (isset($settings['email']) ? $settings['email'] : '') }}">
					        </div>
			    		</div>
			    		{{-- mobile --}}
			    		<div class="col-md-4">
			    			<div class="form-group">
					            ...
					            <input type="text" class="form-control" id="mobile" name="mobile" value="{{ old('mobile') ?? (isset($settings['mobile']) ? $settings['mobile'] : '') }}">
					        </div>
			    		</div>
			    		<div class="col-md-8">
			    			<div class="form-group">
					            <label for="site_name_ar">{{ t('Location','site')}}</label>
					            <input type="text" class="form-control" id="location" name="location" value="{{ old('location') ?? (isset($settings['location']) ? $settings['location'] : '') }}">
					        </div>
			    		</div>
			    		<div class="col-md-6">
			    			{{-- vat --}}
			    			<div class="form-group">
					            ...
					            <input type="number"  class="form-control" id="vat" name="vat" value="{{ old('location') ?? (isset($settings['vat']) ? $settings['vat'] : '') }}">
					        </div>
			    		</div>
			    	...
			    	<div class="row">
			    		<div class="col-md-6">
			    			<div class="form-group">
					            ...
					            <input type="text" class="form-control" id="facbook_link" name="facbook_link" value="{{ old('facbook_link') ?? (isset($settings['facbook_link']) ? $settings['facbook_link'] : '') }}">
					        </div>
			    		</div>
			    		<div class="col-md-6">
					        <div class="form-group">
					            <label for="twitter_link">{{ t('Twitter Link')}}</label>
					            <input type="text" class="form-control" id="twitter_link" name="twitter_link" value="{{ old('twitter_link') ?? (isset($settings['twitter_link']) ? $settings['twitter_link'] : '') }}">
					        </div>
			    		</div>
			    		<div class="col-md-6">
			    			<div class="form-group">
					            <label for="instagram_link">{{ t('Instagram Link')}}</label>
					            <input type="text" class="form-control" id="instagram_link" name="instagram_link" value="{{ old('instagram_link') ?? (isset($settings['instagram_link']) ? $settings['instagram_link'] : '') }}">
					        </div>
			    		</div>
			    		<div class="col-md-6">
				    		<div class="form-group">
				            <label for="maroof_link">{{ t('Maroof Link','site')}}</label>
				            <input type="text" class="form-control" id="maroof_link" name="maroof_link" value="{{ old('maroof_link') ?? (isset($settings['maroof_link']) ? $settings['maroof_link'] : '') }}">
				        </div>
				    </div>
                	...
                	<div class="form-group">
						...
						<input id="kt_tagify_1" class="form-control tagify" name='site_key_words' value="{{ old('site_key_words') ?? (isset($settings['site_key_words']) ? $settings['site_key_words'] : '') }}" />
					</div>
					<div class="row">
							<div class="col-md-12">
								<div class="form-group">
                                <label for="inputAddress">كود خريطة جزء تواصل معنا (Iframe)</label>
                                <textarea dir="ltr" name="contact_us_map_html" class="form-control" id="" cols="30" rows="10">{{ $settings['contact_us_map_html'] }}</textarea>
                            </div>
		            ...
		            <div class="input-group mb-3 half-width">
		                ...
		                <div class="custom-file">
		                    <input id="imgInp" type="file" name="logo" class="custom-file-input" accept="image/x-png,image/gif,image/jpeg" id="imgInp" />
		                    <label class="custom-file-label" for="inputGroupFile01">{{ __('auth.Choose file') }}</label>
		                </div>
		            </div>
		            @if(isset($settings) && $settings['logo'])
		                <div class="text-center">
		                    <img class="img-thumbnail" style="width: 400px;margin-bottom: 20px;" src="{{ url($settings['logo']) }}" alt="admin"width="120" id="blah">
		                </div>
		            @else
		                <div class="text-center">
